...
Bun is the recommended way to use Homunculus packages. Currently the caching features are tied to Bun SQLite, so for now Node.js is supported.

<Tabs groupId='package-manager' persist items={['bun']} label="From a new empty project folder">
