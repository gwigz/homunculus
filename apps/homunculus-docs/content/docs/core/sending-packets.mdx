...
assert(client.self?.position, "Cannot send sound trigger self");
assert(client.region, "Cannot send sound trigger without region");

client.send([
  new Packets.SoundTrigger({
    soundData: {
