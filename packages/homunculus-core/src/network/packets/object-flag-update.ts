...
class ObjectFlagUpdate extends Packet {
		...
		[
			"agentData",
			{
				...
				parameters: new Map<string, any>([
					["agent", Types.UUID],
					["session", Types.UUID],
					["objectLocal", Types.U32],
					["usePhysics", Types.Boolean],
					["isTemporary", Types.Boolean],
					["isPhantom", Types.Boolean],
					["castsShadows", Types.Boolean],
				]),
			},
		],
	])
	...
	/**
	 * ObjectFlagUpdate constructor, can be passed either a fully
	 * initialized Packet Buffer or an object containing this Objects required
	 * parameters from {@link ObjectFlagUpdate.format}. Note that
	 ...
	 * @param {boolean} [data.agentData.usePhysics] UsePhysics
	 * @param {boolean} [data.agentData.isTemporary] IsTemporary
