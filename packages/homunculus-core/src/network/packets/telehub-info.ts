 ...
 * TelehubInfo Packet
 */
class TelehubInfo extends Packet {
	/**
	...
	public static format: Map<string, any> = new Map([
		...
		[
			"spawnPointBlock",
			{
				parameters: new Map<string, any>([["spawnPointPos", Types.Vector3]]),
			},
		],
	])

	/**
	 * TelehubInfo constructor, can be passed either a fully
	 * initialized Packet Buffer or an object containing this Objects required
	 * parameters from {@link TelehubInfo.format}. Note that
	 * "agentData" blocks may be excluded if {@link build} is able to fetch the
	 * requirements itself.
	 *
	 * @param {object|Buffer} [data] Packet block data to be serialized, may be optional
	 * @param {string} [data.telehubBlock.object] ObjectID
