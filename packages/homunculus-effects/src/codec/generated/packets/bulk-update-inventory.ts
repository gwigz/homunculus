...
export const id = 281
export const name = "BulkUpdateInventory"
...
export function encode(sequence: number, reliable: boolean, data: DeepRequired<BulkUpdateInventoryData>) {
	let size = BASE_SIZE
...
export function decode(buffer: Buffer): BulkUpdateInventoryData {
	...
	return {
		...
		folderData: (() => {
			const items: BulkUpdateInventoryData["folderData"] = []
		...
		itemData: (() => {
			const items: BulkUpdateInventoryData["itemData"] = []
			const size = Primitives.U8.decode(buffer, state)
