/**
 * ChatFromSimulator Packet
 *
 * This file is used to help our packet serialization and deserialization
 * process, and to create new packets on the fly.
 *
 * ⚠️ Do not edit this file manually, it is generated by the `codegen` script!
 *
 * @see {@link http://wiki.secondlife.com/wiki/Message_Layout}
 */

import type { DeepRequired } from "ts-essentials"
import * as PacketEncoder from "~/codec/lludp/packet-encoder"
import * as Primitives from "~/codec/primitives"
import type * as Types from "~/model/types"

export interface ChatFromSimulatorData {
	chatData: {
		fromName: Buffer
		sourceId: Types.UUID
		ownerId: Types.UUID
		sourceType: number
		chatType: number
		audible: number
		position: Types.Vector3
		message: Buffer
	}
}

export const id = 139
export const name = "ChatFromSimulator"
export const frequency = 2

const HEADER_SIZE = PacketEncoder.FREQUENCY_OFFSETS[frequency]!

// base byte size for one "chatData" entry (fixed-length parameters only)
const CHAT_DATA_BASE_SIZE =
	Primitives.UUID.size() + // sourceId
	Primitives.UUID.size() + // ownerId
	Primitives.U8.size() + // sourceType
	Primitives.U8.size() + // chatType
	Primitives.U8.size() + // audible
	Primitives.Vector3.size() // position

// size contributed by the packet header and all FIXED-LENGTH fields
const BASE_SIZE =
	// Header
	HEADER_SIZE + CHAT_DATA_BASE_SIZE

export function encode(sequence: number, reliable: boolean, data: DeepRequired<ChatFromSimulatorData>) {
	let size = BASE_SIZE

	size += Primitives.Variable1.size(data.chatData.fromName)
	size += Primitives.Variable2.size(data.chatData.message)

	const buffer = Buffer.allocUnsafe(size)

	PacketEncoder.encodeHeader(buffer, id, frequency, sequence, reliable)

	let offset = HEADER_SIZE

	// ChatData
	offset = Primitives.Variable1.encode(data.chatData.fromName, buffer, offset)
	offset = Primitives.UUID.encode(data.chatData.sourceId, buffer, offset)
	offset = Primitives.UUID.encode(data.chatData.ownerId, buffer, offset)
	offset = Primitives.U8.encode(data.chatData.sourceType, buffer, offset)
	offset = Primitives.U8.encode(data.chatData.chatType, buffer, offset)
	offset = Primitives.U8.encode(data.chatData.audible, buffer, offset)
	offset = Primitives.Vector3.encode(data.chatData.position, buffer, offset)
	offset = Primitives.Variable2.encode(data.chatData.message, buffer, offset)

	return buffer
}

export function decode(buffer: Buffer): ChatFromSimulatorData {
	const state = { offset: HEADER_SIZE }

	return {
		chatData: {
			fromName: Primitives.Variable1.decode(buffer, state),
			sourceId: Primitives.UUID.decode(buffer, state),
			ownerId: Primitives.UUID.decode(buffer, state),
			sourceType: Primitives.U8.decode(buffer, state),
			chatType: Primitives.U8.decode(buffer, state),
			audible: Primitives.U8.decode(buffer, state),
			position: Primitives.Vector3.decode(buffer, state),
			message: Primitives.Variable2.decode(buffer, state),
		},
	}
}
