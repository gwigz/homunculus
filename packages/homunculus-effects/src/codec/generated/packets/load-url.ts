/**
 * LoadURL Packet
 *
 * This file is used to help our packet serialization and deserialization
 * process, and to create new packets on the fly.
 *
 * ⚠️ Do not edit this file manually, it is generated by the `codegen` script!
 *
 * @see {@link http://wiki.secondlife.com/wiki/Message_Layout}
 */

import type { DeepRequired } from "ts-essentials"
import * as PacketEncoder from "~/codec/lludp/packet-encoder"
import * as Primitives from "~/codec/primitives"
import type * as Types from "~/model/types"

export interface LoadURLData {
	data: {
		objectName: Buffer
		objectId: Types.UUID
		ownerId: Types.UUID
		ownerIsGroup: boolean
		message: Buffer
		url: Buffer
	}
}

export const id = 194
export const name = "LoadURL"
export const frequency = 2

const HEADER_SIZE = PacketEncoder.FREQUENCY_OFFSETS[frequency]!

// base byte size for one "data" entry (fixed-length parameters only)
const DATA_BASE_SIZE =
	Primitives.UUID.size() + // objectId
	Primitives.UUID.size() + // ownerId
	Primitives.Boolean.size() // ownerIsGroup

// size contributed by the packet header and all FIXED-LENGTH fields
const BASE_SIZE =
	// Header
	HEADER_SIZE + DATA_BASE_SIZE

export function encode(sequence: number, reliable: boolean, data: DeepRequired<LoadURLData>) {
	let size = BASE_SIZE

	size += Primitives.Variable1.size(data.data.objectName)
	size += Primitives.Variable1.size(data.data.message)
	size += Primitives.Variable1.size(data.data.url)

	const buffer = Buffer.allocUnsafe(size)

	PacketEncoder.encodeHeader(buffer, id, frequency, sequence, reliable)

	let offset = HEADER_SIZE

	// Data
	offset = Primitives.Variable1.encode(data.data.objectName, buffer, offset)
	offset = Primitives.UUID.encode(data.data.objectId, buffer, offset)
	offset = Primitives.UUID.encode(data.data.ownerId, buffer, offset)
	offset = Primitives.Boolean.encode(data.data.ownerIsGroup, buffer, offset)
	offset = Primitives.Variable1.encode(data.data.message, buffer, offset)
	offset = Primitives.Variable1.encode(data.data.url, buffer, offset)

	return buffer
}

export function decode(buffer: Buffer): LoadURLData {
	const state = { offset: HEADER_SIZE }

	return {
		data: {
			objectName: Primitives.Variable1.decode(buffer, state),
			objectId: Primitives.UUID.decode(buffer, state),
			ownerId: Primitives.UUID.decode(buffer, state),
			ownerIsGroup: Primitives.Boolean.decode(buffer, state),
			message: Primitives.Variable1.decode(buffer, state),
			url: Primitives.Variable1.decode(buffer, state),
		},
	}
}
