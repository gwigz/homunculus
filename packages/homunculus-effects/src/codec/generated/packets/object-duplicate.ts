...
export const id = 90
export const name = "ObjectDuplicate"
...
// base byte size for one "sharedData" entry (fixed-length parameters only)
const SHARED_DATA_BASE_SIZE =
...
const BASE_SIZE =
	// Header
	HEADER_SIZE + AGENT_DATA_BASE_SIZE + SHARED_DATA_BASE_SIZE

export function encode(sequence: number, reliable: boolean, data: DeepRequired<ObjectDuplicateData>) {
	let size = BASE_SIZE
	...
	// SharedData
	offset = Primitives.Vector3.encode(data.sharedData.offset, buffer, offset)
	offset = Primitives.U32.encode(data.sharedData.duplicateFlags, buffer, offset)
...
export function decode(buffer: Buffer): ObjectDuplicateData {
	...
	return {
		...
		objectData: (() => {
			const items: ObjectDuplicateData["objectData"] = []
