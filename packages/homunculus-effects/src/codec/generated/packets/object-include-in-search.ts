...
export const id = 424
export const name = "ObjectIncludeInSearch"
...
const OBJECT_DATA_BASE_SIZE =
	...
	Primitives.Boolean.size() // includeInSearch
...
export function encode(sequence: number, reliable: boolean, data: DeepRequired<ObjectIncludeInSearchData>) {
	let size = BASE_SIZE
	...
	for (const item of data.objectData ?? []) {
		...
		offset = Primitives.Boolean.encode(item.includeInSearch, buffer, offset)
	}

	return buffer
}

export function decode(buffer: Buffer): ObjectIncludeInSearchData {
	...
	return {
		...
		objectData: (() => {
			const items: ObjectIncludeInSearchData["objectData"] = []
			const size = Primitives.U8.decode(buffer, state)

			for (let i = 0; i < size; i++) {
				items.push({
					objectLocalId: Primitives.U32.decode(buffer, state),
					includeInSearch: Primitives.Boolean.decode(buffer, state),
				})
			}
