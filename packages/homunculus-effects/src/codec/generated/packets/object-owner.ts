...
const HEADER_DATA_BASE_SIZE =
	Primitives.Boolean.size() + // override
...
export function encode(sequence: number, reliable: boolean, data: DeepRequired<ObjectOwnerData>) {
	let size = BASE_SIZE
	...
	offset = Primitives.UUID.encode(data.headerData.ownerId, buffer, offset)
	offset = Primitives.UUID.encode(data.headerData.groupId, buffer, offset)
...
export function decode(buffer: Buffer): ObjectOwnerData {
	...
	return {
		...
		objectData: (() => {
			const items: ObjectOwnerData["objectData"] = []
