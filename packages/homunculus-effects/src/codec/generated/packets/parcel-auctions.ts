...
export const id = 234
export const name = "ParcelAuctions"
...
export function encode(sequence: number, reliable: boolean, data: DeepRequired<ParcelAuctionsData>) {
	let size = BASE_SIZE
	...
	for (const item of data.parcelData ?? []) {
		...
		offset = Primitives.UUID.encode(item.winnerId, buffer, offset)
	}

	return buffer
}

export function decode(buffer: Buffer): ParcelAuctionsData {
	const state = { offset: HEADER_SIZE }

	return {
		parcelData: (() => {
			const items: ParcelAuctionsData["parcelData"] = []
			const size = Primitives.U8.decode(buffer, state)

			for (let i = 0; i < size; i++) {
				items.push({
					parcelId: Primitives.UUID.decode(buffer, state),
					winnerId: Primitives.UUID.decode(buffer, state),
				})
			}
