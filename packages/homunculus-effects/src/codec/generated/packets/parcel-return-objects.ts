...
export interface ParcelReturnObjectsData {
	agentData?: {
		agentId?: Types.UUID
...
export const id = 199
export const name = "ParcelReturnObjects"
...
export function encode(sequence: number, reliable: boolean, data: DeepRequired<ParcelReturnObjectsData>) {
	let size = BASE_SIZE
...
export function decode(buffer: Buffer): ParcelReturnObjectsData {
	...
	return {
		...
		taskIDs: (() => {
			const items: ParcelReturnObjectsData["taskIDs"] = []
		...
		ownerIDs: (() => {
			const items: ParcelReturnObjectsData["ownerIDs"] = []
			const size = Primitives.U8.decode(buffer, state)
