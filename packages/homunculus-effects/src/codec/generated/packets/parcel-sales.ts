...
export function encode(sequence: number, reliable: boolean, data: DeepRequired<ParcelSalesData>) {
	let size = BASE_SIZE
	...
	for (const item of data.parcelData ?? []) {
		...
		offset = Primitives.UUID.encode(item.buyerId, buffer, offset)
	}

	return buffer
}

export function decode(buffer: Buffer): ParcelSalesData {
	const state = { offset: HEADER_SIZE }

	return {
		parcelData: (() => {
			const items: ParcelSalesData["parcelData"] = []
			const size = Primitives.U8.decode(buffer, state)

			for (let i = 0; i < size; i++) {
				items.push({
					parcelId: Primitives.UUID.decode(buffer, state),
					buyerId: Primitives.UUID.decode(buffer, state),
				})
			}
