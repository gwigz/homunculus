...
export const id = 267
export const name = "UpdateCreateInventoryItem"
...
const AGENT_DATA_BASE_SIZE =
	...
	Primitives.Boolean.size() + // simApproved
...
export function encode(sequence: number, reliable: boolean, data: DeepRequired<UpdateCreateInventoryItemData>) {
	let size = BASE_SIZE
	...
	offset = Primitives.Boolean.encode(data.agentData.simApproved, buffer, offset)
...
export function decode(buffer: Buffer): UpdateCreateInventoryItemData {
	const state = { offset: HEADER_SIZE }

	return {
		agentData: {
			agentId: Primitives.UUID.decode(buffer, state),
			simApproved: Primitives.Boolean.decode(buffer, state),
			transactionId: Primitives.UUID.decode(buffer, state),
		},
		inventoryData: (() => {
			const items: UpdateCreateInventoryItemData["inventoryData"] = []
