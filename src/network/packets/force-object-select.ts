...
class ForceObjectSelect extends Packet {
	...
	public static format: Collection<string, any> = new Collection([
		...
		[
			...
			{
				quantity: 1,
				parameters: new Collection<string, any>([["resetList", Types.Boolean]]),
			},
		],
		// tslint:disable-next-line:max-line-length
		[
			"data",
			{ parameters: new Collection<string, any>([["local", Types.U32]]) },
		],
	])

	/**
	 * ForceObjectSelect constructor, can be passed either a fully
	 * initialized Packet Buffer or an object containing this Objects required
	 * parameters from {@link ForceObjectSelect.format}. Note that
