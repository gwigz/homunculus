...
class MergeParcel extends Packet {
	...
	public static format: Collection<string, any> = new Collection([
		...
		[
			...
			{
				quantity: 1,
				parameters: new Collection<string, any>([["master", Types.UUID]]),
			},
		],
		// tslint:disable-next-line:max-line-length
		[
			"slaveParcelData",
			{ parameters: new Collection<string, any>([["slave", Types.UUID]]) },
		],
	])

	/**
	 * MergeParcel constructor, can be passed either a fully
	 * initialized Packet Buffer or an object containing this Objects required
	 * parameters from {@link MergeParcel.format}. Note that
